...
const { requireUser, setPrismaClient: setPrismaClientForUserMiddleware } = require('./middlewares/requireUser');
...
setPrismaClientForUserMiddleware(prisma);

app.use(express.json());

// app.use(requireUser);
// app.use(requireDocente);
...
const publicacionRouter = publicacionRoutes(prisma);
const composerRouter = composerRoutes;
...
app.get('/api/admin/dashboard-counts', requireAdmin, async (req, res) => {
  try {
    ...
    const pendingTasks = await prisma.tarea.count({ where: { estado: 'ENTREGADA' } });
    const gradedTasks = await prisma.tarea.count({ where: { estado: 'CALIFICADA' } });
...
app.post('/api/catedras', requireAdmin, async (req, res) => {
  try {
    const { nombre, anio, institucion, turno, horario, aula, dias, docenteId } = req.body;
...
app.post('/api/alumnos/verify-otp', async (req, res) => {
  ...
  try {
    ...
    const token = jwt.sign(
      { userId: alumno.id, email: alumno.email, role: 'student', alumnoId: alumno.id },
    ...
    res.json({ token, alumnoId: alumno.id, role: 'student' });
  } catch (error) {
...
app.get('/api/alumnos/me', requireUser, async (req, res) => {
  try {
...
app.get('/api/alumnos/me/contributions', requireUser, async (req, res) => {
  try {
...
app.get('/api/alumnos/:alumnoId/tareas', requireUser, async (req, res) => {
  try {
    const alumnoId = parseInt(req.params.alumnoId);

    // Asegurarse de que el alumno logueado es el mismo que el solicitado o es un admin
    if (req.user.role !== 'admin' && req.user.alumnoId !== alumnoId) {
      return res.status(403).json({ error: 'Acceso denegado: No autorizado para ver estas tareas.' });
    }
...
app.get('/api/tareas/:tareaId', requireUser, async (req, res) => {
  try {
...
app.post('/api/tareas/:tareaId/submit', requireUser, upload.single('file'), async (req, res) => {
  try {
    const tareaId = parseInt(req.params.tareaId);
    const { alumnoId, role } = req.user;
