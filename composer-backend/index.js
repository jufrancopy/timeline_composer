...
app.post('/api/login', async (req, res) => {
  ...
  // En un futuro, esto debería consultar la tabla User
  ...
  try {
    ...
    const composers = await prisma.composer.findMany({
      ...
      select: { // Seleccionar explícitamente los campos que queremos, incluyendo photo_url
        ...
        photo_url: true,
        youtube_link: true,
        completeness_score: true, // <-- AÑADIR ESTA LÍNEA
...
app.post('/api/composers', async (req, res) => {
  try {
    const { first_name, last_name, birth_year, death_year, bio, notable_works, period, email, references, photo_url, youtube_link, mainRole } = req.body;
    ...
    const composer = await prisma.composer.create({
      data: {
        first_name, last_name, birth_year, death_year, bio, notable_works, period, email, references, photo_url, youtube_link, ip_address,
        completeness_score, // Guardar el puntaje
        mainRole: {
...
app.get('/api/ranking', async (req, res) => {
  try {
    ...
    const ranking = contributors.map(c => {
...
// Rechazar un compositor (solo admin)
app.post('/api/composers/:id/reject', requireAdmin, async (req, res) => {
  try {
    const composerId = parseInt(req.params.id, 10);
    const { rejection_reason } = req.body;

    if (!composerId || !rejection_reason) {
      return res.status(400).json({ error: 'ID del compositor y motivo de rechazo son obligatorios' });
    }
          ...
          <div style="font-family: Arial, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; border: 1px solid #ddd; border-radius: 8px; overflow: hidden;">
            ...
            <div style="padding: 20px;">
              ...
              <p style="margin-top: 25px;">
                Por favor, tómate un momento para revisar estos puntos. Una vez que los ajustes estén listos, puedes volver a enviar tu aporte. ¡Estamos ansiosos por verlo publicado!
              </p>
            </div>
            <div style="background-color: #f8f8f8; padding: 15px; text-align: center; font-size: 12px; color: #999;">
              Este es un correo autom��tico. Tu contribución es muy valiosa para nosotros.
            </div>
          </div>
        `,
      };
    ...
    res.json({ message: 'Compositor rechazado correctamente', composer: updatedComposer });
  } catch (error) {
    console.error('Error al rechazar el compositor:', error);
    res.status(500).json({ error: 'Error al rechazar el compositor' });
  }
});
