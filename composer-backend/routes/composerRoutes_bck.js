...
module.exports = (prisma, transporter) => {
    ...
    router.get('/day', async (req, res) => {
        try {
            // Calcular la fecha actual en formato YYYY-MM-DD para usar como semilla
            const today = new Date();
            const dateString = today.toISOString().slice(0, 10); // "YYYY-MM-DD"

            // Generar una semilla numérica a partir de la fecha para consistencia diaria
            let seed = 0;
            ...
            if (!composerOfTheDay) {
                // Si por alguna razón el skip calculó un índice inválido (raro), intentar con el primero
    ...
    router.get('/random', async (req, res) => {
        ...
        try {
            ...
            const totalRatings = randomComposer.Rating ? randomComposer.Rating.length : 0;
            const sumRatings = (randomComposer.Rating || []).reduce((sum, r) => sum + r.rating_value, 0);
            const rating_avg = totalRatings > 0 ? parseFloat((sumRatings / totalRatings).toFixed(1)) : 0.0;
    ...
    router.get('/comments/:composerId', async (req, res) => {
        try {
            ...
            console.error('[ComposerRoutes] Error al obtener comentarios para composerId', parsedComposerId, ':', error);
    ...
    router.post('/ratings', async (req, res) => {
        try {
            ...
            // Calcular el nuevo promedio y total de ratings
            const allRatingsForComposer = await prisma.rating.findMany({
                where: { composerId: parsedComposerId },
                select: { rating_value: true },
            });

            const totalRatings = allRatingsForComposer.length;
            const sumRatings = allRatingsForComposer.reduce((sum, r) => sum + r.rating_value, 0);
            const new_average_rating = totalRatings > 0 ? parseFloat((sumRatings / totalRatings).toFixed(1)) : 0.0;

            res.status(201).json({ new_average_rating, total_ratings: totalRatings });
        } catch (error) {
    ...
    router.get('/:id', async (req, res) => {
        ...
        try {
            ...
            const composer = await prisma.Composer.findUnique({
                ...
                include: {
                    Comment: true, // Cambiado de 'comments' a 'Comment'
                    EditSuggestion: true, // Cambiado de 'editSuggestions' a 'EditSuggestion'
