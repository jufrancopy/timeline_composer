...
router.get('/catedras/:catedraId/publicaciones', requireUser(prisma), async (req, res) => {
    ...
    const { catedraId } = req.params;
    const { role, alumnoId } = req.user; // Desestructurar role y alumnoId de req.user
    try {
        const publicaciones = await prisma.publicacion.findMany({
            where: {
                catedraId: parseInt(catedraId),
                // Lógica de visibilidad para publicaciones:
                // Los administradores ven todo.
                // Los docentes ven todo.
                // Los alumnos solo ven publicaciones no-TAREA o TAREAS con visibleToStudents: true.
                AND: [
                    {
                        OR: [
                            { tipo: { not: 'TAREA' } }, // Siempre muestra publicaciones que no son tareas
                            ...(role === 'alumno' ? [{
                                tipo: 'TAREA',
                                ...
                                { tipo: 'TAREA' }, // Docentes/admins ven todas las tareas maestras
                                { tipo: 'EVALUACION' } // Docentes/admins ven todas las evaluaciones maestras
                            ])
                        ]
                    }
                ],
            ...
            include: {
                ...
                autorAlumno: { // Incluir también al autor alumno si existe
                    select: {
                        id: true,
                        nombre: true,
                        apellido: true,
                ...
                comentarios: {
                    include: {
                        autorAlumno: { // Incluir al autor alumno si el comentario es de un alumno
                            select: {
                                id: true,
                                nombre: true,
                                apellido: true,
                        ...
                        autorDocente: { // Incluir al autor docente si el comentario es de un docente
                            select: {
                                id: true,
                                nombre: true,
                                apellido: true,
                ...
                interacciones: {
                    where: {
                        OR: [
                            req.user.alumnoId ? { alumnoId: req.user.alumnoId } : undefined,
                            req.user.docenteId ? { docenteId: req.user.docenteId } : undefined,
                        ].filter(Boolean), // Filtra los undefined
                    },
                    select: { id: true }, // Solo necesitamos saber si existe una interacción
                },
                _count: {
                    select: { interacciones: true },
                },
                tareaMaestra: { // Incluir la TareaMaestra para verificar asignaciones
                    include: {
                        asignaciones: role === 'alumno' ? {
                            ...
                            select: { alumnoId: true } // Para el docente, solo necesitamos saber quiénes están asignados
                        },
                    },
                },
                evaluacionMaestra: { // Incluir la EvaluacionMaestra
                    include: {
                        catedra: { select: { nombre: true } },
                        // No incluir preguntas/opciones aquí para no sobrecargar el tablón
                    },
                },
                evaluacionAsignacion: role === 'alumno' ? { // Incluir la EvaluacionAsignacion para alumnos
                    where: { alumnoId: alumnoId },
                    include: {
                        calificacion: true, // Para el estado de la calificación
                        evaluacion: { select: { id: true, titulo: true, catedraId: true } }, // Datos básicos de la maestra
                    },
                } : {
                    include: { alumno: true, calificacion: true, evaluacion: { select: { id: true, titulo: true } } }, // Docentes/admins ven detalles de asignaciones
                }
            },
            orderBy: {
                created_at: 'desc',
    ...
    try {
        ...
        const nonTaskPublicaciones = await prisma.publicacion.findMany({
            ...
            include: { /* ... includes necesarios ... */ 
                ...
                comentarios: { include: { autorAlumno: { select: { id: true, nombre: true, apellido: true, email: true } }, autorDocente: { select: { id: true, nombre: true, apellido: true, email: true } } }, orderBy: { created_at: 'asc' } },
...
router.put('/publicaciones/:publicacionId/toggle-visibility', requireDocenteOrAdmin, async (req, res) => {
    ...
    try {
        ...
        if (req.user.role === 'docente' && publicacion.autorDocenteId !== req.user.docenteId && publicacion.catedraId !== req.user.catedraId) {
