...
function DocenteEvaluationResultsPage() {
  ...
  useEffect(() => {
    const fetchResults = async () => {
      try {
        setLoading(true);
        const response = await api.getEvaluationResults(catedraId, alumnoId, evaluationId);
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="bg-white/5 backdrop-blur-lg p-8 rounded-lg shadow-xl">
          ...
          <p className="text-center text-gray-300 mb-8">
            Alumno: {results.alumnoNombre} {results.alumnoApellido} | Calificación: {results.score} / {results.totalPoints}
          </p>
